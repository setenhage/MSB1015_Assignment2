...
3b) To be able to test how well the model I train performs, it is necessary to keep a part of our data aside as testing data. In the following code you can easily change the size of the test data by adjusting percentage_test. 
...
set.seed(50) 
sample = sample.split(full_data, SplitRatio = percentage_test)
train = subset(full_data, sample == FALSE)
test  = subset(full_data, sample == TRUE)
...
#Create null_model
null_model <- mean(train$data.bp)

#Get error of null model
error_null_model <- rmse(test$data.bp, null_model)
...
PLS_model <- plsr(data.bp ~ (apol + WPATH + WPOL), 
                  data = train, 
                  validation = "CV"
                  )
...
plot(test$data.bp, predicted.bp[,,3], xlim=c(250,1000), ylim=c(250,1000))
